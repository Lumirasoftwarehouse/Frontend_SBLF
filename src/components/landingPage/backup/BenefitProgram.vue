<template>
  <div class="template" id="benefit">
    <div class="container">
      ...
      <div class="row">
        <div class="col-md-4 mb-4" v-for="(benefit, index) in dataBenefits" :key="index">
          <div class="card" data-aos="fade-up" data-aos-duration="1000">
            <div class="card-img-overlay">
              <h5 class="card-title text-white">Benefit {{ index + 1 }}</h5>
              <p class="card-text text-white">{{ benefit.benefit }}</p>
            </div>
          </div>
        </div>
      </div>
...
.card {
  background-color: rgba(0, 0, 0, 0.5); /* Ubah opacity dan warna overlay sesuai kebutuhan */
  border: none;
  border-radius: 10px;
  color: #fff; /* Warna teks di card */
  height: 200px; /* Sesuaikan tinggi card */
}
