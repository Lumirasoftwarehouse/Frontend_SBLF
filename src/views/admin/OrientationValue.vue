                ...
                <table class="display table table-striped" v-if="ready">
                  ...
                  <tbody>
                    <tr v-for="(item, index) in orientationValues" :key="item.id">
                      ...
                      <td>
                         <img
                          :src="`http://127.0.0.1:8000/storage/orientationValue/${item.image}`"
                          alt="trend"
                          width="200"
                        />
                        </td> 
    ...
    async updateValue() {
      try {
        ...
        const response = await axios.post(
          `http://127.0.0.1:8000/api/auth/update-orientation/${this.idTrend}`,
          formData,
          {
            headers: {
      ...
      try {
        ...
        const response = await axios.post(
          "http://127.0.0.1:8000/api/auth/create-orientation",
          formData,
          {
            headers: {
              Authorization: `Bearer ${sessionStorage.getItem("token")}`
    ...
    async deleteTrend(id) {
      try {
        const response = await axios.delete(
          `http://127.0.0.1:8000/api/auth/delete-orientation/${id}`,
          {
            headers: {
              Authorization: `Bearer ${sessionStorage.getItem("token")}`,
            },
    ...
    async fetchTrends() {
      ...
      try {
        const response = await axios.get(
          "http://127.0.0.1:8000/api/auth/list-orientation",
          {
            headers: {
              Authorization: `Bearer ${sessionStorage.getItem("token")}`
            }
